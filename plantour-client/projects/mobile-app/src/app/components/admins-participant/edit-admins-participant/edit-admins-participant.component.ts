...
export class EditAdminsParticipantComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      if (this.participantId) {
        ...
        this.messagesService.showError('Participant ID not provided');
  ...
  private loadParticipant(): void {
    this.isLoading = true;
    this.adminsParticipantService.getById(this.participantId).subscribe({
      next: (participant) => {
      ...
      error: (error) => {
        ...
        this.messagesService.showError('Failed to load participant');
        this.router.navigate(['/admins-participant']);
      }
    });
  }
  ...
  onSubmit(): void {
    ...
    this.adminsParticipantService.update(request).subscribe({
      next: () => {
        this.messagesService.showInfo('Participant updated successfully');
        this.router.navigate(['/admins-participant']);
      },
      error: (error) => {
        console.error('Error updating participant:', error);
        this.messagesService.showError('Failed to update participant');
