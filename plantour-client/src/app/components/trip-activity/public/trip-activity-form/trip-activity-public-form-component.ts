...
@Component({
  selector: 'app-trip-activity-public-form',
  standalone: true,
  imports: [
    CommonModule,
    InputTextModule,
  ...
  templateUrl: './trip-activity-public-form-component.html',
  styleUrl: './trip-activity-public-form-component.scss',
})
export class TripActivityPublicFormComponent implements OnInit {
  ...
  get title(): string {
    return `${capitalizeFirstLetter(this.mode)} Public Trip Activity`;
  }
  ...
  ngOnInit(): void {
    ...
    if (!this.tripId) {
      throw new Error('Trip Id is required to create or edit a public trip activity');
    }
    ...
    if (!this.id) {
      throw new Error('Id is required to edit a public trip activity');
    }
    this.loadActivity();
  }
  ...
  private loadActivity(): void {
    ...
    this.service.getPublicById(this.tripId!, this.id).subscribe({
  ...
  private addActivity() {
    ...
    this.service.addPublic(request).subscribe({
      next: (activity: TripActivityDto) => {
        this.localStorageService.setComponentKey('trip-activities-public', 'selectedId', activity.id);
        this.messagesService.showInfo('Public trip activity added successfully');
  ...
  private updateActivity(): void {
    ...
    this.service.updatePublic(request).subscribe({
      next: () => {
        this.localStorageService.setComponentKey('trip-activities-public', 'selectedId', this.id!);
        this.messagesService.showInfo('Public trip activity updated successfully');
        this.router.navigate([this.tripActivitiesUrl]);
      },
    });
  }
  ...
  get tripActivitiesUrl(): string {
    return `/trips/${this.tripId}/trip-activities/public`;
  }
}