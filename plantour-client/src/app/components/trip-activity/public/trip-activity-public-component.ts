...
import { TripActivityPublicItemComponent } from './trip-activity-item/trip-activity-public-item-component';
import { enrichTripActivities, sortTripActivityItineraryParts } from '../trip-activity-utils';

@Component({
  selector: 'app-trip-activity-public',
  standalone: true,
  imports: [
  ...
  templateUrl: './trip-activity-public-component.html',
  styleUrl: './trip-activity-public-component.scss',
})
export class TripActivityPublicComponent implements OnInit {
  tripActivityItemComponent = TripActivityPublicItemComponent;
  componentId = 'trip-activities-public';
  ...
  ngOnInit(): void {
    ...
    this.itineraryService.getAll(this.tripId).pipe(
      ...
      switchMap((parts: ItineraryPartDto[]) =>
        this.tripActivityService.getAllPublic(this.tripId!).pipe(
  ...
  deleteTripActivity(id: string): void {
    this.tripActivityService.deletePublic(id).pipe(
  ...
  assignItineraryPart(entity: TripActivityDto, itineraryPartId: string | null): void {
    ...
    this.tripActivityService.updatePublic(request).pipe(
      switchMap(() => this.itineraryService.getAll(this.tripId!)),
      map((parts: ItineraryPartDto[]) => sortTripActivityItineraryParts(parts)),
      switchMap((parts: ItineraryPartDto[]) =>
        this.tripActivityService.getAllPublic(this.tripId!).pipe(
